...
const BitString BitString::POW5_N100 = {
    false,
    {
        0x914E7FA6,
        0x3E359E29,
...
BitString::BitString() : sign(false), mantissa(1,0), exponent(0) {}

BitString::BitString(const bool sign, const std::vector<uint32_t>& mantissa, int64_t exponent)
    : sign(sign), mantissa(mantissa), exponent(exponent) {}

BitString::BitString(const BitString& other) : sign(other.sign), mantissa(other.mantissa), exponent(other.exponent) {}

BitString::BitString(const string& str) {
    *this = fromString(str);
}

BitString::BitString(const int32_t n) : sign(n < 0), mantissa(1, (uint32_t)(sign ? -n : n)), exponent(0) {
    normalize();
}
...
BitString BitString::operator+(const BitString& other) const {
    ...
    // Express lower magnitude number in exponent of higher's
    if (a.exponent > b.exponent) swap(a, b);
    int64_t shift = b.exponent - a.exponent;
    leftShift(a.mantissa, shift);
    ...
    if (a.sign == b.sign) {
        ...
        // Ensure subtraction happens "greater - lower", tweak sign accordingly
        if (cmp < 0) swap(great, low);
        result.sign = great->sign;
        result.mantissa = great->mantissa;

        uint64_t borrow = 0; // basically carry but for sub
...
BitString BitString::operator*(const BitString& b) const {//TODO: this is O(n^2), migrate to Karatsuba later
    BitString result;
...
bool BitString::operator!=(const BitString& o) const { return !(*this==o); }
...
bool BitString::operator>(const BitString& o) const { return o<*this; }
